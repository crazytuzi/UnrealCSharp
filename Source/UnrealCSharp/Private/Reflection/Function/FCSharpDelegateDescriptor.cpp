...
bool FCSharpDelegateDescriptor::CallDelegate(const UObject* InObject, MonoObject* InDelegate, void* InParams)
{
	...
	for (auto Index = 0; Index < PropertyDescriptors.Num(); ++Index)
	{
		if (const auto PropertyDescriptor = PropertyDescriptors[Index])
		{
			...
			PropertyDescriptor->Get<std::false_type>(PropertyDescriptor->ContainerPtrToValuePtr<void>(InParams), &Object);
			
			CSharpParams[Index] = PropertyDescriptor->IsPrimitiveProperty() ? FCSharpEnvironment::GetEnvironment().GetDomain()->Object_Unbox(static_cast<MonoObject*>(Object)) : Object;
		}
	}
