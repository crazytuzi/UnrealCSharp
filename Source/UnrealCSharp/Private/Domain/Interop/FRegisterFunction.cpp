...
namespace
{
	...
	{
		static void GenericCall0Implementation(const IManagedHandle InManagedHandle,
		...
		static void PrimitiveCall1Implementation(const IManagedHandle InManagedHandle,
		...
		static void CompoundCall1Implementation(const IManagedHandle InManagedHandle,
		                                        const uint32 InFunctionHash, RETURN_BUFFER_SIGNATURE)
		{
			if (const auto FoundObject = FCSharpEnvironment::GetEnvironment().GetObject(InManagedHandle))
			{
		...
		static void GenericCall2Implementation(const IManagedHandle InManagedHandle,
		...
		static void PrimitiveCall3Implementation(const IManagedHandle InManagedHandle,
		                                         const uint32 InFunctionHash, IN_BUFFER_SIGNATURE,
		...
		static void CompoundCall3Implementation(const IManagedHandle InManagedHandle,
		                                        const uint32 InFunctionHash, IN_BUFFER_SIGNATURE,
		                                        RETURN_BUFFER_SIGNATURE)
		{
			if (const auto FoundObject = FCSharpEnvironment::GetEnvironment().GetObject(InManagedHandle))
		...
		static void GenericCall4Implementation(const IManagedHandle InManagedHandle,
		...
		static void PrimitiveCall7Implementation(const IManagedHandle InManagedHandle,
		                                         const uint32 InFunctionHash, IN_BUFFER_SIGNATURE, OUT_BUFFER_SIGNATURE,
		                                         RETURN_BUFFER_SIGNATURE)
		{
			if (const auto FoundObject = FCSharpEnvironment::GetEnvironment().GetObject(InManagedHandle))
		...
		static void CompoundCall7Implementation(const IManagedHandle InManagedHandle,
		                                        const uint32 InFunctionHash, IN_BUFFER_SIGNATURE, OUT_BUFFER_SIGNATURE,
		                                        RETURN_BUFFER_SIGNATURE)
		{
			if (const auto FoundObject = FCSharpEnvironment::GetEnvironment().GetObject(InManagedHandle))
		...
		static void GenericCall8Implementation(const IManagedHandle InManagedHandle,
		                                       const uint32 InFunctionHash)
		{
			if (const auto FoundObject = FCSharpEnvironment::GetEnvironment().GetObject(InManagedHandle))
			{
		...
		static void PrimitiveCall9Implementation(const IManagedHandle InManagedHandle,
		                                         const uint32 InFunctionHash, RETURN_BUFFER_SIGNATURE)
		{
			if (const auto FoundObject = FCSharpEnvironment::GetEnvironment().GetObject(InManagedHandle))
			{
		...
		static void CompoundCall9Implementation(const IManagedHandle InManagedHandle,
		                                        const uint32 InFunctionHash, RETURN_BUFFER_SIGNATURE)
		{
			if (const auto FoundObject = FCSharpEnvironment::GetEnvironment().GetObject(InManagedHandle))
			{
		...
		static void GenericCall10Implementation(const IManagedHandle InManagedHandle,
		                                        const uint32 InFunctionHash, IN_BUFFER_SIGNATURE)
		{
			if (const auto FoundObject = FCSharpEnvironment::GetEnvironment().GetObject(InManagedHandle))
			{
		...
		static void PrimitiveCall11Implementation(const IManagedHandle InManagedHandle,
		                                          const uint32 InFunctionHash, IN_BUFFER_SIGNATURE,
		                                          RETURN_BUFFER_SIGNATURE)
		{
			if (const auto FoundObject = FCSharpEnvironment::GetEnvironment().GetObject(InManagedHandle))
		...
		static void CompoundCall11Implementation(const IManagedHandle InManagedHandle,
		                                         const uint32 InFunctionHash, IN_BUFFER_SIGNATURE,
		                                         RETURN_BUFFER_SIGNATURE)
		{
			if (const auto FoundObject = FCSharpEnvironment::GetEnvironment().GetObject(InManagedHandle))
		...
		static void GenericCall14Implementation(const IManagedHandle InManagedHandle,
		                                        const uint32 InFunctionHash, IN_BUFFER_SIGNATURE, OUT_BUFFER_SIGNATURE)
		{
			if (const auto FoundObject = FCSharpEnvironment::GetEnvironment().GetObject(InManagedHandle))
			{
		...
		static void PrimitiveCall15Implementation(const IManagedHandle InManagedHandle,
		                                          const uint32 InFunctionHash, IN_BUFFER_SIGNATURE,
		...
		static void CompoundCall15Implementation(const IManagedHandle InManagedHandle,
		                                         const uint32 InFunctionHash, IN_BUFFER_SIGNATURE, OUT_BUFFER_SIGNATURE,
		                                         RETURN_BUFFER_SIGNATURE)
		{
			if (const auto FoundObject = FCSharpEnvironment::GetEnvironment().GetObject(InManagedHandle))
		...
		static void GenericCall16Implementation(const IManagedHandle InManagedHandle,
		                                        const uint32 InFunctionHash)
		{
			if (const auto FoundObject = FCSharpEnvironment::GetEnvironment().GetObject(InManagedHandle))
			{
		...
		static void GenericCall18Implementation(const IManagedHandle InManagedHandle,
		                                        const uint32 InFunctionHash, IN_BUFFER_SIGNATURE)
		{
			if (const auto FoundObject = FCSharpEnvironment::GetEnvironment().GetObject(InManagedHandle))
			{
		...
		static void GenericCall24Implementation(const IManagedHandle InManagedHandle,
		                                        const uint32 InFunctionHash)
		{
			if (const auto FoundObject = FCSharpEnvironment::GetEnvironment().GetObject(InManagedHandle))
			{
		...
		static void GenericCall26Implementation(const IManagedHandle InManagedHandle,
		                                        const uint32 InFunctionHash, IN_BUFFER_SIGNATURE)
		{
			if (const auto FoundObject = FCSharpEnvironment::GetEnvironment().GetObject(InManagedHandle))
			{
